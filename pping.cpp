...
static void process_packet(const Packet& pkt)
{
    ...
    std::time_t result = pkt.timestamp().seconds(); // Pocinje meriti vreme u programu
    if (offTm < 0) {
        offTm = static_cast<int64_t>(pkt.timestamp().seconds()); //Prvi paket koji dodje na port
        // fractional part of first usable packet time
        startm = double(pkt.timestamp().microseconds()) * 1e-6; // Pocetno vreme merenja
        capTm = startm; //
        if (sumInt) {
        ...
        capTm = double(tt) + double(pkt.timestamp().microseconds()) * 1e-6; // Vreme kad je primljen paket setuje vreme
    }

    std::string fstr = srcstr + "+" + dststr;  // could add DSCP field to key // Src portovi i destination portovi
    ...
    if (flows.count(fstr) == 0u) {
        ...
        fr = new flowRec(fstr); // Novi podaci sa tim i tim portovima
        flowCnt++;
        flows.emplace(fstr, fr); // Formira se nova mapa tokova sa tim i tim portovima i odgovarajucim Flow-tokom
    ...
    fr->last_tm = capTm; //Zapisuje vrijeme kada je uhvaćen trenutni paket (u tom toku) u odnosu na prvi poslani paket 

    if (! fr->revFlow) { // Unidirekcioni smer, znaci jos se nije pojavio tok u suprtnom smeru sa istim dst i src portovima 
        uniDir++;
        return;
    }

    double arr_fwd = fr->bytesSnt + pkt.pdu()->size(); // bajti svih  paketa ukljucujuci i trenutni(za isti flow)
    fr->bytesSnt = arr_fwd; // Refresuje vrednost sa novom kada dodje novi paket
    if (!filtLocal || (localIP != ipdstr)) { // Ako nema filtriranja ili ako paket nije namenjen nasoj IP adresi
        addTS(fstr + "+" + std::to_string(rcv_tsval),
              new tsInfo(capTm, arr_fwd, fr->bytesDep)); // Pravi TimeStamp src i dst port dodaje sval, capmtm, isti flow, svi flow-ovi
    }
    tsInfo* ti = getTStm(dststr + "+" + srcstr + "+" +
                         std::to_string(rcv_tsecr)); // Uzima se iz mape na osnovu key vrednosti(suprotan smjer tog toka)
    if (ti && ti->t > 0.0) {
	// this packet is the return "pping" --
        // process it for packet's src
        double t = ti->t; // Uzima captm u sustini
        double rtt = capTm - t; // Trenutni - prethodni 
        if (fr->min > rtt) {
